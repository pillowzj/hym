...
public class TokenService
{
     ...
     * @Author klj
     */
    public LoginUser psrseUser(String token) {
        // 获取请求携带的令牌
        if (StringUtils.isNotEmpty(token)){
            Claims claims = parseToken(token);
            // 解析对应的权限以及用户信息
    ...
    public void verifyToken(LoginUser loginUser)
    {
        ...
        {
            System.out.println("----refreshToken---->"+(expireTime - currentTime));
            refreshToken(loginUser);
        }
    }
...
//        loginUser.setExpireTime(loginUser.getLoginTime() + 12 * 60 * MILLIS_MINUTE); // 5min
        // 根据uuid将loginUser缓存
        String userKey = getTokenKey(loginUser.getToken());
...
//        redisCache.setCacheObject(userKey, loginUser, 12*60, TimeUnit.MINUTES); // 5min
    }
